            ...
            <div className="flex flex-1 flex-col gap-6 p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Manage employee records, salaries, and department assignments.</p>
                    </div>
                ...
                {employees.data.length === 0 ? (
                    <Card>
                        <CardContent className="flex flex-col items-center gap-3 py-16 text-center">
                            ...
                            <p className="text-sm text-muted-foreground max-w-md">Add employees to manage payroll, assignments, and department budgets.</p>
                            <Button onClick={openNew} className="mt-2"><Plus className="mr-2 h-4 w-4" /> Add your first employee</Button>
                        </CardContent>
                    </Card>
                ) : (
                    <div className="overflow-x-auto rounded-lg border">
                        <table className="w-full text-sm">
                            <thead className="border-b bg-muted/50">
                                <tr>
                                    <th className="px-4 py-3 text-left font-medium text-muted-foreground">Emp#</th>
                            ...
                            <tbody className="divide-y">
                                {employees.data.map((e) => (
                                    <tr key={e.id} className="bg-background hover:bg-muted/30 transition-colors">
                                        <td className="px-4 py-3 font-mono text-xs font-medium">{e.employee_number}</td>
                                        ...
                                        <td className="px-4 py-3">{formatCurrency(e.base_salary)}</td>
                                        <td className="px-4 py-3">
                ...
                {employees.last_page > 1 && (
                    <div className="flex items-center justify-between text-sm text-muted-foreground">
                        <span>{employees.from}–{employees.to} of {employees.total}</span>
...
function EmployeeDialog({
    ...
    const { data, setData, post, put, processing, errors, reset } = useForm({
        ...
        employment_type: editing?.employment_type_label ?? (employment_types[0]?.value ?? ''),
        salary_type: editing?.salary_type_label ?? (salary_types[0]?.value ?? ''),
